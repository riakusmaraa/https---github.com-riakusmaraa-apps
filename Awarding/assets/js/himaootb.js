...
function fetchNominies() {
    ...
    //     })
    //proker andalan
    axios.get(BASEURL + 'vote/vote/nomini/99cf54ca-1b1b-4591-acb8-7a9a44a378cd')
        .then(({ data }) => {
            nominies = data.data
            renderNominies(nominies)
        })
...
function renderNominies(nominies) {
    ...
    var filteredNominies = nominies.filter(nomini => {
        return nomini.description == 'TOP5_HIMA_Proker OOTB'
    })
                ...
                Swal.fire({
                  ...
                  if (result.isConfirmed) {
                      ...
                      .then(res => {
                        ...
                        if(`${res.data.message}` == 'kamu sudah melakukan voting 	TOP5_HIMA_Proker OOTB'){
                          Swal.fire(
                            'Maaf!',
                            `${res.data.message}`,
                            'error'
                        ...
                        }if(`${res.data.message}` == 'voting TOP5_HIMA_Proker OOTB telah ditutup pada 2020-11-21 09:59:00'){
                          Swal.fire(
                            'Maaf!',
                            `${res.data.message}`,
                            'error'
