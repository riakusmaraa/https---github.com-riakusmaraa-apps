...
function fetchNominies() {
    ...
    //     })
    axios.get(BASEURL + 'vote/vote/nomini/a31d3f98-aaf3-4c00-acd6-0db37cc293de')
        .then(({ data }) => {
            nominies = data.data
            renderNominies(nominies)
        })
...
function renderNominies(nominies) {
    ...
    var filteredNominies = nominies.filter(nomini => {
        return nomini.description == 'TOP5_BEM_Kepuasan Publik'
    })
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        .then(res => {
                            ...
                            if(`${res.data.message}` == 'kamu sudah melakukan voting TOP5_BEM_Kepuasan Publik'){
                              Swal.fire(
                                'Maaf!',
                                `${res.data.message}`,
                                'error'
                            ...
                            }if(`${res.data.message}` == 'voting TOP5_BEM_Kepuasan Publik telah ditutup pada 2020-11-21 09:59:00'){
                              Swal.fire(
                                'Maaf!',
                                `${res.data.message}`,
                                'error'
