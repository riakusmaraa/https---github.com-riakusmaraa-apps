...
function fetchNominies() {
    ...
    axios.get(BASEURL + 'vote/vote/nomini/19f0605c-1414-4f76-aa97-af1045a226d5')
        .then(({ data }) => {
            nominies = data.data
            renderNominies(nominies)
        })
...
function renderNominies(nominies) {
    ...
    var filteredNominies = nominies.filter(nomini => {
        return nomini.description == 'TOP5_DPM_Teraspiratif'
    })
                  ...
                  if (result.isConfirmed) {
                      ...
                      .then(res => {
                          Swal.fire(
                            ...
                            if (`${res.data.mesaage}` == 'kamu sudah melakukan voting TOP5_DPM_Teraspiratif'){
                              Swal.fire(
                                'Maaf',
                                `${res.data.message}`,
                                'eror'
                            ...
                            if (`${res.data.mesaage}` == 'kamu bukan mahasiswa ub ya?'){
                              Swal.fire(
                                'Maaf',
                                `${res.data.message}`,
                                'eror'
                            ).then(function() {
                              window.location.href = "https://em.ub.ac.id/apps/profile.html";
                             })
                            }if(`${res.data.message}` == 'voting TOP5_DPM_Teraspiratif telah ditutup pada 2020-11-21 09:59:00'){
                              Swal.fire(
                                'Maaf!',
                                `${res.data.message}`,
                                'error'
