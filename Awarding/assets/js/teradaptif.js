...
function fetchNominies() {
    ...
    //     })
    axios.get(BASEURL + 'vote/vote/nomini/abe924d2-65b2-4c99-a8b7-194d0cb7e05b')
        .then(({ data }) => {
            nominies = data.data
            renderNominies(nominies)
        })
...
function renderNominies(nominies) {
    ...
    var filteredNominies = nominies.filter(nomini => {
        return nomini.description == 'TOP5_BEM_Teradaptif dan Inovatif'
    })
                ...
                Swal.fire({
                  ...
                  if (result.isConfirmed) {
                      ...
                      .then(res => {
                        ...
                        if(`${res.data.message}` == 'kamu sudah melakukan voting TOP5_BEM_Teradaptif dan Inovatif'){
                          Swal.fire(
                            'Maaf!',
                            `${res.data.message}`,
                            'error'
                        ...
                        }if(`${res.data.message}` == 'voting TOP5_BEM_Teradaptif dan Inovatif telah ditutup pada 2020-11-21 09:59:00'){
                          Swal.fire(
                            'Maaf!',
                            `${res.data.message}`,
                            'error'
